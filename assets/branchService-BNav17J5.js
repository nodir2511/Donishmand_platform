import{s as n}from"./index-D3LUehMy.js";import{c as f}from"./cleanUndefined-DcGDzFdA.js";const u={async getTeacherClasses(e){if(!e)return[];const{data:r,error:a}=await n.from("classes").select(`
                id, 
                name, 
                created_at,
                branch_id,
            ...
            `).eq("teacher_id",e);if(c)throw c;const s=t.map(i=>i.classes).filter(i=>i),o=[...r,...s],l=Array.from(new Map(o.map(i=>[i.id,i])).values());return l.sort((i,d)=>new Date(d.created_at)-new Date(i.created_at)),l.map(i=>{var d,h;return{...i,studentsCount:((h=(d=i.class_members)==null?void 0:d[0])==null?void 0:h.count)||0}})},async getStudentClasses(e){if(!e)return[];const{data:r,error:a}=await n.from("class_members").select(`
                joined_at,
                classes!inner (
                    id, 
                    name, 
            ...
            `).eq("student_id",e);if(a)throw a;const t=r.map(s=>({...s.classes,teacher:s.classes.teacher,joinedAt:s.joined_at}));if(t.filter(s=>!s.teacher&&s.teacher_id).map(s=>s.teacher_id).length>0)try{const{data:s,error:o}=await n.rpc("get_all_profiles");!o&&s&&t.forEach(l=>{if(!l.teacher&&l.teacher_id){const i=s.find(d=>d.id===l.teacher_id);i&&(l.teacher={id:i.id,full_name:i.full_name,role:i.role,avatar_url:i.avatar_url})}})}catch(s){console.error("Failed to fetch missing teachers via RPC:",s)}return t},async getClassDetails(e){if(!e)return null;const{data:r,error:a}=await n.from("classes").select(`
                id, 
                name, 
                description,
                invite_code,
            ...
            `).eq("id",e).single();if(a)throw a;if(!r)return null;if(!r.teacher&&r.teacher_id)try{const{data:t,error:c}=await n.rpc("get_all_profiles");if(!c&&t){const s=t.find(o=>o.id===r.teacher_id);s&&(r.teacher={id:s.id,full_name:s.full_name,role:s.role,avatar_url:s.avatar_url})}}catch(t){console.error("Failed to fetch teacher profile via RPC in getClassDetails:",t)}return r},async createClass(e,r,a=null,t=null){if(!e||!r)throw new Error("Имя класса и ID учителя обязательны");const{data:c,error:s}=await n.from("classes").insert([{name:e.trim(),teacher_id:r,branch_id:a||null,subject_id:t||null}]).select(`
                *,
                branch:branches (id, name)
